/**
 * BatchBeneficiaries10000.test.js — Large Scale Performance Test (10000 Beneficiaries)
 * DPA Foundation — OPAL Platform
 *
 * Tests batch payment processing at scale: 10000 beneficiaries.
...
describe("Batch Beneficiaries — 10000 Scale Tests", function () {
    this.timeout(1200000); // 20 minutes — very large-scale test
    ...
    const TOTAL_BENEFICIARIES = 10000;
    const BATCH_SIZE = 50;  // MAX_BATCH_SIZE in the contract
    const TOTAL_BATCHES = TOTAL_BENEFICIARIES / BATCH_SIZE; // 200 batches
    const AMOUNT_PER_BENEFICIARY = 5000; // 5 000 CFA
    const TOTAL_AMOUNT = AMOUNT_PER_BENEFICIARY * TOTAL_BENEFICIARIES; // 50 000 000 CFA
    ...
    before(async function () {
        console.log("\n  ╔══════════════════════════════════════════════════╗");
        console.log("  ║  OPAL Platform — 10000 Beneficiaries Scale Test  ║");
        console.log("  ╚══════════════════════════════════════════════════╝\n");
        ...
        console.log("  [2/3] Generating 10000 beneficiaries & Merkle tree...");
    ...
    // =====================================================================
    describe("Merkle Tree — 10000 Beneficiaries", function () {
        it("should generate valid Merkle root from 10000 leaves", function () {
            expect(merkleRoot).to.not.equal(ethers.ZeroHash);
            expect(tree.getLeafCount()).to.equal(TOTAL_BENEFICIARIES);
        });

        it("should have correct tree depth for 10000 leaves", function () {
            // ceil(log2(10000)) = 14
        ...
        it("should verify Merkle proofs for 20 sampled beneficiaries", function () {
            const indices = [0, 100, 500, 999, 1500, 2000, 3000, 4000, 5000,
                             6000, 7000, 7500, 8000, 8500, 9000, 9500, 9999,
                             42, 1337, 2718];
            let verifiedCount = 0;
    ...
    // =====================================================================
    //   BATCH PAYMENTS — 10000 BENEFICIARIES (single trigger)
    // =====================================================================
    describe("Batch Payments — 10000 Beneficiaries", function () {
        ...
        it("should process all 10000 beneficiaries in 200 batches of 50", async function () {
            this.timeout(1200000);

            console.log("\n  Processing 10000 beneficiaries (200 batches × 50)...\n");
            ...
            console.log("\n  ═══════════════════════════════════════════════");
            console.log("  RESULTS — 10000 Beneficiaries");
            console.log("  ═══════════════════════════════════════════════");
        ...
        it("should verify payment status for 20 sampled beneficiaries", async function () {
            const indices = [0, 50, 200, 999, 1500, 2500, 3500, 4500, 5500,
                             6500, 7500, 8000, 8500, 9000, 9200, 9500, 9999,
                             42, 1337, 2718];
            let verifiedCount = 0;
    ...
    //   SYSTEM STATISTICS
    // =====================================================================
    describe("System Stats — Post 10000 Beneficiaries", function () {
        it("should report correct total payments after processing 10000 beneficiaries", async function () {
    ...
    // =====================================================================
    describe("Duplicate Prevention at 10000 Scale", function () {
        it("should prevent double-payment for any previously paid beneficiary", async function () {
            ...
            console.log("     ✅ Double-payment correctly prevented at 10000 scale");
        });
    });
});
